...
class ET_Builder_Popup_Trigger_CTA extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Popup Trigger CTA', 'et_builder' );
		$this->slug       = 'et_pb_button_popup';
	...
	function get_fields() {
		$fields = array(
			'popup_id' => array(
				...
				'description'     => esc_html__( 'Add the ID number of the popup you wish to display (This can be found under Popup Maker > All Popups > under CSS classes there will be a number after "popmake-". This is the number to use here)', 'et_builder' ),
			),
			'button_text' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$popup_id        = $this->shortcode_atts['popup_id'];
		...
		$output = sprintf(
			'<div class="et_pb_button_module_wrapper et_pb_module%8$s">
				<a class="et_pb_button%5$s%7$s popmake-%1$s  pum-trigger" data-do-default="" style="cursor: pointer;" href="#"%3$s%4$s%6$s>%2$s</a>
			</div>',
			esc_html( $popup_id ),
			'' !== $button_text ? esc_html( $button_text ) : esc_html( $popup_id ),
...
new ET_Builder_Popup_Trigger_CTA;